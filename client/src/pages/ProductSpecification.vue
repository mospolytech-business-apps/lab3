...
const { fetchProductSpecifications, updateProductSpecification } =
...
onMounted(async () => {
    productSpecifications.value = allProductSpecifications.value.length
...
const editSpecification = (id) => {
    const specification = productSpecifications.value.find(
        (el) => el.id === id
    );
    ...
    <main class="main">
        <ul v-for="specification in productSpecifications">
            <div class="header">
                ...
                >
                <UIButton v-else @click="saveEdited(specification.id)"
                    >Сохранить</UIButton
                >
            </div>
            ...
            <li>
                <h4>
                    ...
                    <span v-if="specification.semi_finished_products == null"
                        >–</span
                    >
                </h4>
                ...
                <ol v-if="specification.semi_finished_products != null">
                    <li
                        v-for="semi_finished_product in specification.semi_finished_products"
                    >
                        <span v-if="!specification.being_edited">{{
                            semi_finished_product.name
                        }}</span>
                        <input
                            v-else
                            type="text"
                            v-model="semi_finished_product.name"
                        />
                    </li>
                </ol>
            </li>
            ...
            <li>
                ...
                <ol>
                    <li v-for="operation in specification.operations">
                        <i v-if="!specification.being_edited">{{
                            operation.name
                        }}</i>
                        ...
                        <ul>
                            <li>
                                ...
                                <span v-if="!specification.being_edited">{{
                                    operation.type_equipment
                                }}</span>
                                <input
                                    v-else
                                    type="text"
                                    v-model="operation.type_equipment"
                                />
                            </li>
                            <li>
                                ...
                                <template v-if="!specification.being_edited">
                                    <span>{{
                                        operation.time_for_operation.amount
                                    }}</span>
                                    <span>&nbsp;</span>
                                    <!-- пробел -->
                                    <span>{{
                                        operation.time_for_operation.units
                                    }}</span>
                                </template>
